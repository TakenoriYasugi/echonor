    ...
    <>
      ...
      >
        <Typography
          ...
          textAlign={"center"}
        >
          ※現在EchoNorはメンテナンス中です。再開までしばらくお待ちください
        </Typography>
        <StyledButton
          variant={"contained"}
          sx={{ m: 1 }}
        ...
        <Typography color={"red"} fontSize={12}>
          ※ゲスト(未登録のユーザー)はタイムラインの閲覧のみ行えます。
        </Typography>
        <Typography color={"red"} fontSize={12}>
          投稿やリアクションにはユーザー登録が必要です。
        </Typography>
      </Stack>

      <Typography variant="body1" sx={{ m: 3 }} textAlign={"center"}>
        EchoNor（エコノア）は「繋がらないSNS」というコンセプトの下開発中の、匿名性を重視したSNSです。
        <br />
      ...
      >
        {contents.map((content, index) => (
          ...
          >
            <CardContainer key={index}>{content}</CardContainer>
          </Grid>
        ))}
      </Grid>
    </>
